...
class FeatureExtractorBackbone(Backbone):
    ...
    def single_forward(self, img, depth1,depth2,depth3,name_list):

        # save memory
        input_image_size = img.shape[-2:]
        img = self.image_preprocess(img)
        depth1 = self.image_preprocess(depth1)
        depth2 = self.image_preprocess(depth2)
        depth3 = self.image_preprocess(depth3)

        img = ImageList.from_tensors(list(img), self.size_divisibility).tensor
        depth1 = ImageList.from_tensors(list(depth1), self.size_divisibility).tensor
        depth2 = ImageList.from_tensors(list(depth2), self.size_divisibility).tensor
        depth3 = ImageList.from_tensors(list(depth3), self.size_divisibility).tensor

        features = self.feature_extractor(dict(img=img,depth1=depth1,depth2=depth2,depth3=depth3,name_list=name_list))
    ...
    def slide_forward(self, img,depth1,depth2,depth3,name_list):
        ...
        for h_idx in range(h_grids):
            for w_idx in range(w_grids):
                ...
                crop_depth1 = depth1[:, :, y1:y2, x1:x2]
                crop_depth2 = depth2[:, :, y1:y2, x1:x2]
                crop_depth3 = depth3[:, :, y1:y2, x1:x2]

                assert crop_img.shape[-2:] == (h_crop, w_crop), f"{crop_img.shape} from {img.shape}"
                crop_features = self.single_forward(crop_img,crop_depth1,crop_depth2,crop_depth3,name_list)
    ...
    def forward(self, img,depth1,depth2,depth3,name_list):
        if (self.training and not self._slide_training) or not self._slide_inference:
            return self.single_forward(img,depth1,depth2,depth3,name_list)
        else:
            return self.slide_forward(img,depth1,depth2,depth3,name_list)
