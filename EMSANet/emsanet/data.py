...
def get_dataset_pddm(split):
    dataset_name = 'nyuv2'
    dataset_path = "./EMSANet/datasets/nyuv2"
    dataset_depth_mode = 'refined'
    cache_dataset = False
 
    # determine sample keys
    sample_keys = ['rgb','depth_meta','depth_mari','depth_anything','semantic']
...
class DataHelper:
    def __init__(
        ...
        self._dataloaders_valid = DataLoader(
                datasets_valid,
        ...
        self._overfitting_dataset = deepcopy(self.datasets_valid)
    ...
    def datasets_valid(self) -> Tuple[DatasetType]:
        return self._dataloaders_valid.dataset
    ...
    def set_valid_preprocessor(self, preprocessor):
        self.datasets_valid.preprocessor = preprocessor
...
def get_datahelper_pddm() -> DataHelper:
    # get datasets
    dataset_train = get_dataset_pddm('train')
    dataset_valid = get_dataset_pddm('test')
    dataset_valid_list = []
    

    if 'test' in dataset_train.SPLITS:
        dataset_test = get_dataset_pddm('test')
        dataset_test_list = []
        for camera in dataset_valid.cameras:
            dataset_camera = deepcopy(dataset_test).filter_camera(camera)
            dataset_test_list.append(dataset_camera)
    ...
    return DataHelper(
        dataset_train=dataset_train,
        subset_train=1.0,
        subset_deterministic=False,
        batch_size_train=2, 
        datasets_valid=dataset_valid,#dataset_valid_list,
        batch_size_valid=2,
        datasets_test=dataset_test_list,
        n_workers=8,
        persistent_worker=False,     # only if caching is enabled
    )
