...
class NYUv2(NYUv2Meta, RGBDDataset):
    ...
    def _load_depth_anything(self, idx) -> np.array:
        return self._load(self.DEPTH_ANYTHING_DIR, self._filenames[idx])
    def _load_depth_meta(self, idx) -> np.array:
        return self._load(self.DEPTH_META_DIR, self._filenames[idx])
    def _load_depth_mari(self, idx) -> np.array:
        return self._load(self.DEPTH_MARI_DIR, self._filenames[idx])

    def _load_identifier(self, idx: int) -> Tuple[str]:
        return SampleIdentifier((self._filenames[idx],))

    def _load_sam_mask(self, idx) -> np.array:
        fp = os.path.join(self._dataset_path,
                          self.split,
                          self.sam_mask_dir,
                          f'{self._filenames[idx]}.npy')
        return np.load(fp)
        # return self._load(self.depth_unidepth_dir, self._filenames[idx])
