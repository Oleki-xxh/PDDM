...
class RunHelper:
    ...
    def validation_step(self, batch, batch_idx):
        ...
        task_loss_dict, task_logs = self._task_helpers[0].validation_step(
            batch=batch,
            batch_idx=batch_idx,
            predictions_post=predictions_post
        )
        ...
        # accumulate losses
        loss = 0#self._loss_weighting_module.reduce_losses(losses, batch_idx)

        # add total loss to logs
        logs['total_loss'] = 0#loss.detach().clone()
...
def main():
    ...
    if args.weights_filepath is not None: ###################################!!!!!!!!!!!!!!!!!!!!!!
        ...
        for key in list(state_dict.keys()):
            new_key = key.replace('panoptic_helper.','')
    ...
    try:
        for epoch in range(args.n_epochs):
            ...
            if ((epoch >= (args.n_epochs * args.validation_skip)) or force) and epoch>=300:
                run.set_inference_mode()
